...
def MLS_one_run(max_passes):
    ...
    while(True):
        ...
        if(flag == 1):
            final_end = time.time()
            final_elpased_time = final_end - new_start
            break
    return run_data_frame, final_elpased_time

def ILS_one_run(max_passes, mls_time):
    flag = 0
    ils_run = 0
    total_passes = 0
    total_time = 0
    ...
    while(True):
        ...
        final_end = time.time()
        final_elpased_time = final_end - new_start
        if(final_elpased_time >= mls_time):
            break
    return run_data_frame,final_elpased_time

def GLS_one_run_no_duplicate(max_passes,mls_time):
    population_size = 50
    num_vertices = 500
    total_passes = 0
    total_time = 0
...
def GLS_one_run(max_passes,mls_time):
    population_size = 50
    num_vertices = 500
    total_passes = 0
    total_time = 0
...
max_passes = 10000
total_runs = 25
mls_run_data_frame = pd.DataFrame(columns = ['Converged local optima','Num_local_optima','Time(s)','Total time'])
ils_run_data_frame = pd.DataFrame(columns = ['Converged local optima','Num_roa','Num_local_optima','Time(s)','Total time'])
gls_run_data_frame = pd.DataFrame(columns = ['Converged local optima','Time(s)','Total time'])
gls_no_duplicate_run_data_frame = pd.DataFrame(columns = ['Converged local optima','Time(s)','Total time'])
for i in range(0,total_runs):
    ...
    observations, ils_time = ILS_one_run(max_passes,mls_time)
    ils_observations = [observations.min()[0],observations.sum()[1],len(observations),observations.max()[2],ils_time]
    ils_run_data_frame.loc[len(ils_run_data_frame)] = ils_observations
    print("ILS",ils_observations)
    
    observations = GLS_one_run_no_duplicate(max_passes,mls_time)
    best_local_optimum = observations[0]
    total_time = observations[1]
    gls_time = observations[2]
    gls_observations = [best_local_optimum,total_time,gls_time]
    print("GLS_no_duplicate",gls_observations)
    gls_run_data_frame.loc[len(gls_run_data_frame)] = gls_observations
    
    observations = GLS_one_run_normal(max_passes,mls_time)
    best_local_optimum = observations[0]
    total_time = observations[1]
    gls_time = observations[2]
    gls_observations = [best_local_optimum,total_time,gls_time]
    print("GLS",gls_observations)
